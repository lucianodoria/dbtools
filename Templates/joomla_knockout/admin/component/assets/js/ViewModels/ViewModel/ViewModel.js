...
/// <reference path="../../libs/knockout/knockout-3.4.2.debug.js" />
...
DoriaTI.{PREFIX}.MvcApplication.ViewModels.{TYPE}.IndexViewModel = function () {
            ...
            {FIELDS_COLUMNS}
            {
                headerText: "Ações", rowText:
                {
                    columnButtons: [
                        ...
                        { iconCssClass: "fas fa-trash-alt color-danger", textCssClass: 'color-danger', text: 'Excluir', title: 'Clique para Excluir este registro.', showButtom: function () { return $.PermissaoExcluir; }, buttonClick: function (item) { self.Excluir(item); } }
                    ],
    ...
    self.AbrirModal = function () {
        ...
        if (self.Id() == 0 && self.ModelEdicaoTemp != null) {
            self.ModelEdicao(ko.viewmodel.fromModel(self.ModelEdicaoTemp));
        ...
        $.Ajax{PREFIX} (options, function (response) {
            ...
            if (response != null) {
                ...
                if (self.Id() == 0) {
                    self.ModelEdicaoTemp = novoCliente;
                }

                self.ModelEdicao(ko.viewmodel.fromModel(novoCliente));

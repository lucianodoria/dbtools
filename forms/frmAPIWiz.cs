...
{
    public partial class frmAPIWiz : Form
    {
        ...
        public frmAPIWiz(DBTools.classes.ObjectsClass.Table table, ref SqlObjectsClass so)
        {
            InitializeComponent();
        ...
        private void createBusiness()
        {
            ...
            for (int i = 0; i < _columns.Count; i++)
            {
                    ...
                    _columns[i].DataType.SqlDataType == classes.ObjectsClass.SqlDataType.Numeric)
                {
                    if (_columns[i].Name != txtDateCreate.Text && _columns[i].Name != txtDateUpdate.Text)
                    {
                        rowsConvertsType.Add(string.Format("$entity->{0} = str_replace('.','', $entity->{0});", _columns[i].Name));
                        rowsConvertsType.Add(string.Format("$entity->{0} = str_replace(',','.', $entity->{0});", _columns[i].Name));
                    }
                }
            }
            ...
            if (rowsQueryJoins.Length > 0)
            {
                if (txtUserCreate.Text.Length > 0)
                {
                    rowsQueryJoins += string.Format("// Usuário que cadastrou{0}" +
                                                    "$query->join('LEFT', '#__hotel_usuarios_usu AS usu ON usu.id_usu = {1}.id_usu');{0}{0}" +
                                                    "$query->select('pes.nome_razao_social_pes AS nome_razao_social_usuario_cadastro'){0}" +
                                                    "->join('LEFT', '#__hotel_pessoas_pes AS pes ON pes.id_pes = usu.id_pes');",
                                                    Environment.NewLine,
                                                    _posfixTableSigla);
                }
